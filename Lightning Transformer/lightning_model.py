...
class LightningTransformer(LightningModule):
    def __init__(self, learning_rate, tokenizer_src, tokenizer_tgt, config = myconfig):
        super().__init__()
        ...
        self.lightningmodel  = build_transformer(
            self.src_vocab_size,
            self.tgt_vocab_size,
            self.seq_len,
            self.seq_len,
    ...
    def forward(self, encoder_input, decoder_input, encoder_mask, decoder_mask):
        ...
        encoder_output = self.lightningmodel.encode(encoder_input, encoder_mask) # (B, seq_len, d_model)
        decoder_output = self.lightningmodel.decode(encoder_output, encoder_mask, decoder_input, decoder_mask) # (B, seq_len, d_model)
        proj_output = self.lightningmodel.project(decoder_output) # (B, seq_len, vocab_size)
        return proj_output
    ...
    def greedy_decode(self, source, source_mask, max_len: int, device: str):
        ...
        # encoder output
        encoder_output = self.lightningmodel.encode(source, source_mask)
        ...
        while True:
            ...
            # build target mask
            decoder_mask = (
                self.causal_mask(decoder_input.size(1)).type_as(source_mask).to(device)
            )
            out = self.lightningmodel.decode(
                encoder_output, source_mask, decoder_input, decoder_mask
            )

            # get next token
            prob = self.lightningmodel.project(out[:, -1])
